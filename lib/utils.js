...
export const downloadFiles = async (filePath, urls) => {
  ...
  for (const source of urls) {
    ...
    const sourceName = typeof source === "string" ? source : source.name || source.url;
    const fallbackFilePath = typeof source === "string" ? undefined : source.fallbackFilePath;
    ...
    try {
      ...
      if (fallbackFilePath && existsSync(fallbackFilePath)) {
        console.warn(
          `Failed to download "${sourceName}" from ${url}. Using fallback file: ${fallbackFilePath}`
        );
        const writeStream = createWriteStream(filePath, { flags: "a" });
        await pipeline(createReadStream(fallbackFilePath), writeStream, {
          end: false,
        });
        writeStream.end("\n");
        continue;
      ...
      throw new Error(`Failed to download required filter source "${sourceName}" (${url})`, { cause: err });
    }
  }
};
...
export const fetchRetry = async (...args) => {
  let attempts = 0;
  let maxAttempts = DOWNLOAD_RETRY_MAX_ATTEMPTS;
  let response;

  while (attempts < maxAttempts) {
    try {
      const requestArgs = [...args];
      const maybeInit = requestArgs[1];
      const init =
        maybeInit && typeof maybeInit === "object" && !Array.isArray(maybeInit)
          ? { ...maybeInit }
          : {};
      if (!init.signal) {
        init.signal = AbortSignal.timeout(DOWNLOAD_HTTP_TIMEOUT_MS);
      }
      requestArgs[1] = init;
