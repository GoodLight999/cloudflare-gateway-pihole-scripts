...
const parsedListChunkLimit = parseInt(process.env.CLOUDFLARE_LIST_CHUNK_LIMIT, 10);
export const LIST_CHUNK_LIMIT = Number.isNaN(parsedListChunkLimit) || parsedListChunkLimit < 1
  ? null
  : parsedListChunkLimit;
...
export const DOWNLOAD_HTTP_TIMEOUT_MS = isNaN(process.env.CGPS_DOWNLOAD_HTTP_TIMEOUT_MS)
  ? 120000
  : parseInt(process.env.CGPS_DOWNLOAD_HTTP_TIMEOUT_MS, 10);
export const DOWNLOAD_RETRY_DELAY_MS = isNaN(process.env.CGPS_DOWNLOAD_RETRY_DELAY_MS)
  ? 5000
  : parseInt(process.env.CGPS_DOWNLOAD_RETRY_DELAY_MS, 10);
export const DOWNLOAD_RETRY_MAX_ATTEMPTS = isNaN(process.env.CGPS_DOWNLOAD_RETRY_MAX_ATTEMPTS)
  ? 50
  : parseInt(process.env.CGPS_DOWNLOAD_RETRY_MAX_ATTEMPTS, 10);
...
const parseUserDefinedSources = (value) =>
  value
    .split("\n")
    .map((x) => x.trim())
    .filter((x) => x)
    .map((x) => {
      const [name, ...urlParts] = x.split("|");
      if (!urlParts.length) return x;
...
export const USER_DEFINED_ALLOWLIST_URLS = process.env.ALLOWLIST_URLS
  ? parseUserDefinedSources(process.env.ALLOWLIST_URLS)
  : undefined;

export const USER_DEFINED_BLOCKLIST_URLS = process.env.BLOCKLIST_URLS
  ? parseUserDefinedSources(process.env.BLOCKLIST_URLS)
  : undefined;
