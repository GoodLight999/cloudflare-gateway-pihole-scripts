...
export const synchronizeZeroTrustLists = async (items) => {
  ...
  if (toAdd.length) {
    ...
    if (cgpsLists.length + requiredExtraLists > maximumAllowedLists) {
      throw new Error(
        `Creating ${requiredExtraLists} additional list(s) would exceed CLOUDFLARE_LIST_CHUNK_LIMIT=${LIST_CHUNK_LIMIT}. Existing CGPS lists: ${cgpsLists.length}.`
      );
    }
