import { BLOCK_PAGE_ENABLED, DEBUG, LIST_CHUNK_LIMIT, LIST_ITEM_SIZE } from "./constants.js";
...
export const synchronizeZeroTrustLists = async (items) => {
  ...
  // Are there any more appends remaining?
  if (toAdd.length) {
    const requiredExtraLists = Math.ceil(toAdd.length / LIST_ITEM_SIZE);
    const maximumAllowedLists = LIST_CHUNK_LIMIT ?? Number.POSITIVE_INFINITY;
    const projectedTotalLists = cgpsLists.length + requiredExtraLists;
    if (projectedTotalLists > maximumAllowedLists) {
      throw new Error(
        `Creating ${requiredExtraLists} additional list(s) would result in ${projectedTotalLists} CGPS lists, exceeding CLOUDFLARE_LIST_CHUNK_LIMIT=${LIST_CHUNK_LIMIT}. Existing CGPS lists: ${cgpsLists.length}.`
      );
    }
